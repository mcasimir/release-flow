...
import ChangelogEntry from "../../src/plugins/generate-changelog/ChangelogEntry";
import plugin from "../../src/plugins/generate-changelog";

describe("plugins", function () {
  describe("generateChangelog", function () {
    afterEach(function () {
      if (fs.existsSync("XYZ.md")) {
        fs.unlinkSync("XYZ.md");
      }
    });
    ...
    describe("getChangelogEntries", function () {
      ...
      it("set root subject after release name", function () {
        let release = new Release({
          changelogPath: "XYZ.md",
          plugins: [plugin],
        });
      ...
      it("for first release sets root subjectLink after release name", function () {
        let release = new Release({
          changelogPath: "XYZ.md",
          plugins: [plugin],
        });
        ...
        stub(release.git, "commitLink").callsFake(
      ...
      it("sets root subjectLink as compareLink between prev and release", function () {
        let release = new Release({
          changelogPath: "XYZ.md",
          plugins: [plugin],
        });
        ...
        stub(release.git, "compareLink").callsFake(
        ...
        step.run(release);
        equal(release.changes.subjectLink, "comparelink://release1..release2");
      });
      ...
      it("adds a list of feat entries if one is present", function () {
        let release = new Release({
          changelogPath: "XYZ.md",
          plugins: [plugin],
        });
      ...
      it("adds a list of fix entries if one is present", function () {
        let release = new Release({
          changelogPath: "XYZ.md",
          plugins: [plugin],
        });
      ...
      it("adds a list of breaking changes entries if one is present", function () {
        let release = new Release({
          changelogPath: "XYZ.md",
          plugins: [plugin],
        });
    ...
    describe("writeChangelog", function () {
      it("prepends changelog contents to changelog", function () {
        ...
        deepEqual(fs.readFileSync("XYZ.md", "utf8"), "abc");
      });
    });
  });
});
