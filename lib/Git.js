...
class Git {
  ...
  localTags() {
    return execCommand(`git --no-pager tag`, {splitLines: true});
  }
  ...
  lastLocalTagSha() {
    return execCommand('git --no-pager log --no-walk --tags --pretty="%h" --decorate=short -1');
  }
  ...
  commits(fromSha, options) {
    ...
    let rawCommits = execCommand(`git --no-pager log --pretty='format:%B%n${HASH_DELIMITER}%n%H${COMMIT_SEPARATOR}' ${range}`).split(COMMIT_SEPARATOR);
    let commits = rawCommits.map(function(rawCommit) {
      return conventionalCommitsParser.sync(rawCommit, options);
    });
